...
export default async function CampaignsPage() {
  ...
  const [campaignsRes, recipientsRes] = await Promise.all([
    ...
    supabase.from("campaign_recipients").select("campaign_id, status").eq("org_id", profile.org_id),
  ]);
  ...
  if (activeCampaigns.length > 0) {
    const totalSent = activeCampaigns.reduce((sum, c) => sum + (recipientsByStatus[c.id]?.sent ?? 0), 0);
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 xl:grid-cols-4">
        ...
        <article className="rf-kpi">
          ...
          <p className="mt-4 text-4xl font-semibold">{totalSent.toLocaleString("fr-FR")}</p>
        </article>

        <article className="rf-kpi">
          <p className="text-sm text-[var(--rf-text-muted)]">Taux d&apos;ouverture moyen</p>
          <p className="mt-4 text-4xl font-semibold">{avgOpenRate.toFixed(1)}%</p>
        </article>

        <article className="rf-kpi">
          <p className="text-sm text-[var(--rf-text-muted)]">Taux de conversion moyen</p>
          <p className="mt-4 text-4xl font-semibold">{avgConversionRate.toFixed(1)}%</p>
        </article>
      </section>
          ...
          <table className="rf-table">
            ...
            <tbody>
              {campaignRows.map((campaign) => {
                ...
                const conversionRate = sent > 0 ? ((clicks / sent) * 100).toFixed(1) : "0.0";
                ...
                return (
                  <tr key={campaign.id}>
                    ...
                    <td>
                      <span className={STATUS_CLASS[campaign.status] ?? "rf-badge"}>{campaign.status}</span>
                    </td>
                    ...
                    <td>
                      <span className="rf-badge">{conversionRate}%</span>
                    </td>
